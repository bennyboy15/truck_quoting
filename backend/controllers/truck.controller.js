...
export async function getTruck(req, res) {
  try {
    const { id } = req.params;
    const truck = await Truck.findById(id).populate("model");
    if (!truck) {
      return res.status(404).json({ message: "Truck not found" });
    }
    return res.status(200).json(truck);
  } catch (error) {
    console.log("Error in getTruck truck controller", error);
    return res.status(500).json({ message: "Internal Server Error" });
  }
}
...
export async function createMake(req, res) {
  try {
    ...
    if (!name || !code) {
      return res
        .status(400)
        .json({ message: "Missing fields, all field are required" });
    }
    ...
    return res
      .status(201)
      .json({ message: "Successfully created new Truck Make" });
  } catch (error) {
...
export async function createModel(req, res) {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Successfully created new Truck Model" });
  } catch (error) {
